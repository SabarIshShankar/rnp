...
import {
  StyleSheet, View, FlatList, Image, Dimensions, ActivityIndicator
} from 'react-native';
...
export default class Images extends Component{
  ...
  getImages(){
    ...
    setTimeout(() => {
      axios({
        url: 'https://api.shutterstock.com/vh/images/search',
        header: {
  ...
  render() {
    if(this.state.loading && this.state.page === 1){
      return (
        ...
        numColumns={3}
        keyExtractor={this._keyExtractor} renderItem={({item}) => (
          <View style={{padding: 5}}>
          <Image style={{height: 100, width: equalWidth}}
